...
async function readChecklists(): Promise<Checklist[]> {
  const rawValue = getItem(STORAGE_KEY);
...
async function writeChecklists(checklists: Checklist[]): Promise<void> {
  setItem(STORAGE_KEY, JSON.stringify(checklists));
}
...
export async function setPendingAccountSaveChecklistId(
  ...
  setItem(PENDING_ACCOUNT_SAVE_KEY, checklistId);
}

export async function consumePendingAccountSaveChecklistId(): Promise<
  string | null
> {
  const checklistId = getItem(PENDING_ACCOUNT_SAVE_KEY);

  if (checklistId) {
    removeItem(PENDING_ACCOUNT_SAVE_KEY);
  }
